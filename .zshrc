...
# wk script to wake up project, opens in new tmux window and runs project specific wake up script
# setting the jobs count here to use in the script
function wk() {
    export jobs_count=$(jobs | wc -l)
    command wk "$@"
}
...
alias gc="git commit"
alias nv="nvim"
# OpenCode mode launcher (corp/hack); defines the `oc` function.
[[ -r ~/.config/opencode/launch.sh ]] && source ~/.config/opencode/launch.sh
# Fast, model-free cross-repo recap (today|week)
